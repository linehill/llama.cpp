...
// All registered devices with default device in the front.
...
// TODO: rename to ggml_cl_platform (current would clash with OCL C++ bindings).
struct cl_platform {
    cl_platform_id     id;
    unsigned           number;
    char               name[128];
...
static void ggml_cl2_init() {
    ...
    cl_device * selected_devices     = nullptr;
    unsigned    n_selected_devices   = 0;
    ...
    if (user_platform_number != -1 && user_device_number != -1) {
        ...
        selected_devices   = platform->devices;
        n_selected_devices = platform->n_devices;
    } else {
        ...
        // Choose a default platform if the user is not choosing one.
        if (user_platform_number == -1) {
            // TODO: Select most suitable platform.
            user_platform_number = 0;
        }

        // Choose a platform by index specified by the user.
        if (user_platform_number != -1) {
    ...
    GGML_ASSERT(n_selected_devices != 0 && selected_devices);

    // Put the default device in front.
    for (unsigned i = 1; i < n_selected_devices; i++) {
        if (&selected_devices[i] == default_device) {
            std::swap(selected_devices[0], *default_device);
            break;
        }
    }

    std::string selected_devices_str;
    for (auto dev = selected_devices, dev_end = selected_devices + n_selected_devices; dev != dev_end; dev++) {
        selected_devices_str += std::string("ggml_opencl:   ") + dev->name + "\n";
    }

    GGML_LOG_INFO("ggml_opencl: selecting platform: '%s'\n", default_device->platform->name);
    GGML_LOG_INFO("ggml_opencl: selected devices:\n%s\n", selected_devices_str.c_str());
    GGML_LOG_INFO("ggml_opencl: selecting default device: '%s'\n", default_device->name);
...
static std::optional<ggml_backend_device> ggml_init_backend_device(const cl_device * target_device, cl_context cl_ctx) {
    ...
    backend_ctx->platform = target_device->platform->id;
    backend_ctx->device   = target_device->id;

    if (strstr(target_device->name, "Adreno")) {
        backend_ctx->gpu_family = GPU_FAMILY::ADRENO;
        backend_ctx->adreno_gen = get_adreno_gpu_gen(target_device->name);

        // Default wave size is 128, A8x uses 64.
        if (backend_ctx->adreno_gen == ADRENO_GPU_GEN::A8X) {
            backend_ctx->adreno_wave_size = 64;
        } else if (backend_ctx->adreno_gen == ADRENO_GPU_GEN::A7X || backend_ctx->adreno_gen == ADRENO_GPU_GEN::X1E) {
            backend_ctx->adreno_wave_size = 128;
        } else {
            backend_ctx->adreno_wave_size = 128;
            GGML_LOG_WARN(
                "ggml_opencl: Unsupported Adreno GPU: %s, "
                "using wave size %d, "
                "may not work as expected\n",
                backend_ctx->device_name.c_str(), backend_ctx->adreno_wave_size);
        }
    } else if (strstr(target_device->name, "Intel")) {
        backend_ctx->gpu_family = GPU_FAMILY::INTEL;
    } else {
        backend_ctx->gpu_family = GPU_FAMILY::UNKNOWN;
        // TODO: Check the device has required capabilities.
    }
    ...
    backend_ctx->platform_name = target_device->platform->name;
    backend_ctx->device_name = target_device->name;
    ...
    // Disabled for PoCL/CPU: 'Cannot find symbol _Z11read_imageh21ocl_image1d_buffer_roi in kernel library'
    ...
    backend_ctx->kernel_mul_mat_q4_0_f32_flat_img_v0 = NULL;
...
static void dump_tensor(ggml_backend_t backend, const struct ggml_tensor * tensor) {
    ...
    ggml_backend_opencl_device_context *backend_ctx = (ggml_backend_opencl_device_context *)backend->context;
    cl_command_queue queue = backend_ctx->queue;
#ifdef GGML_OPENCL_SOA_Q
    void * buf_q;
    void * buf_d;
...
static void ggml_cl_rms_norm(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    size_t sgs;
    CL_CHECK(clGetKernelSubGroupInfo(kernel, backend_ctx->device, CL_KERNEL_MAX_SUB_GROUP_SIZE_FOR_NDRANGE,
